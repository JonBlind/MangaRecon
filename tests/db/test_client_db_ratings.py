...
def test_normalize_score_clamps_and_snaps():
    assert ClientDatabase._normalize_score(-1.0) == 0.0
    assert ClientDatabase._normalize_score(0.0) == 0.0
    assert ClientDatabase._normalize_score(10.6) == 10.0
    assert ClientDatabase._normalize_score(9.74) == 9.5
    assert ClientDatabase._normalize_score(9.76) == 10.0
    assert ClientDatabase._normalize_score(7.26) == 7.5

    with pytest.raises(ValueError):
        ClientDatabase._normalize_score(None)


@pytest.mark.asyncio
async def test_rate_manga_creates_and_updates(client_db, db_session):
...
@pytest.mark.asyncio
async def test_rate_manga_raises_on_none_score(client_db, db_session):
    user = await make_user(db_session)
    manga = await make_manga(db_session)
...
@pytest.mark.asyncio
async def test_get_user_rating_for_manga_none_when_missing(client_db, db_session):
    user = await make_user(db_session)
    manga = await make_manga(db_session)
...
@pytest.mark.asyncio
async def test_get_user_rating_for_manga_returns_when_present(client_db, db_session):
    user = await make_user(db_session)
    manga = await make_manga(db_session)
...
@pytest.mark.asyncio
async def test_get_all_user_ratings_empty_when_none(client_db, db_session):
    user = await make_user(db_session)
...
@pytest.mark.asyncio
async def test_get_all_user_ratings_returns_list(client_db, db_session):
    user = await make_user(db_session)
