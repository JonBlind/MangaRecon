...
async def create_profile(request: schemas.CreateProfileRequest,
    ...
    try:
        ...
        profile_data["password_hash"] = hash_password(profile_data.pop("password"))
 
        return await db.create_profile(profile_data)
    
    except Exception:
        logger.error("Unexpected error during profile creation.", exc_info=True)
        return error(message="Internal Sever Error", detail="Profile creation failed.")
...
async def get_profile_by_email(email:str,
    ...
    try:
        ...
        logger.error("Unexpected error during profile lookup.", exc_info=True)
        return error(message="Internal Sever Error", detail="Email lookup failed.")
